    ...
    stages {
        stage('Setting Up Testing Environment') {
            steps{
                sh '''
                    ...
                    docker run -p 8081:80 --network api_maria --name devops_api marcuskielman/devops_api &
                    '''
            }
        }
        stage('Testing API Docker Image and Network Connection'){
            steps{
                ...
                sh "curl http://0.0.0.0:8081 || docker container stop devops_api maria_db && docker container rm devops_api maria_db"
